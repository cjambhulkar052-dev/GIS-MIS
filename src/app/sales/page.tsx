...
import { SoldTrendChart, type DailySoldCount } from "@/components/sold-trend-chart";
...
export default async function SalesPage({
  ...
  // Oldest -> newest, left to right, so the trend reads the way time passes.
  const trendDates = dates.slice(0, TREND_DAYS).slice().reverse();
  const trendReports = await Promise.all(trendDates.map((d) => getCachedSoldForDate(d)));
  const trendData: DailySoldCount[] = trendDates.map((date, i) => ({
    date,
    count: trendReports[i].noBaseline ? 0 : trendReports[i].soldRows.length,
  }));
        ...
        <div className="mx-auto max-w-7xl px-6 py-8">
          {dates.length === 0 ? (
            ...
            <div className="flex flex-col gap-6">
              {trendData.length > 1 && <SoldTrendChart data={trendData} />}
                    ...
                    <>
                      ...
                      {report.noBaseline ? (
                        <div className="rounded-2xl border border-dashed border-ink-200 bg-white p-12 text-center">
                          ...
                          <p className="mt-1 text-sm text-ink-500">
                            This was the first completed sync at the time, so there&apos;s
                        ...
                        <div className="rounded-2xl border border-dashed border-ink-200 bg-white p-12 text-center">
                          ...
                          <p className="mt-1 text-sm text-ink-500">
                            Every listing available on {formatDate(report.previousDate!)} was
                            still available on {formatDate(report.date)}.
                          </p>
                        </div>
                      ) : (
