...
export default async function CityInventoryPage({
  ...
  const { country: encodedCountry, city: encodedCity } = await params;
  ...
  const universities = summarizeByUniversity(listings);
      ...
      <main className="flex-1 bg-ink-50/40">
        <section className="border-b border-ink-200 bg-white">
          <div className="mx-auto max-w-5xl px-6 py-10">
            ...
            <h1 className="mt-3 text-2xl font-semibold tracking-tight text-ink-900">{city}</h1>
            {latestDate && (
        ...
        <div className="mx-auto max-w-5xl px-6 py-8">
          {mapPoints.length > 0 && (
            <div className="mb-6 overflow-hidden rounded-2xl border border-ink-800 bg-ink-900 shadow-xl shadow-ink-900/20">
              <div className="flex items-center justify-between border-b border-white/10 px-6 py-4">
                <div>
                  <h2 className="text-sm font-semibold text-white">{city} inventory map</h2>
              ...
              <div className="h-[440px]">
                <PropertyMapLoader points={mapPoints} />
              </div>
            </div>
          )}
          ...
          {!latestDate ? (
            ...
            <div className="rounded-2xl border border-dashed border-ink-200 bg-white p-12 text-center">
              ...
              <p className="mt-1 text-sm text-ink-500">
                We don&apos;t have any mapped listings for {city} yet.
              </p>
            </div>
          ) : (
            <div className="rounded-2xl border border-ink-200 bg-white">
              <div className="border-b border-ink-100 px-5 py-4">
                <h2 className="text-sm font-semibold text-ink-900">Universities</h2>
                <p className="mt-0.5 text-xs text-ink-500">
                  Nearest university Amber reports for each property — click one for its inventory
                </p>
              </div>
              <div className="divide-y divide-ink-100">
                {universities.map((u) => (
                  <Link
                    key={u.university}
                    href={`/insights/country/${encodeURIComponent(country)}/city/${encodeURIComponent(city)}/university/${encodeURIComponent(u.university)}`}
                    className="flex items-center justify-between gap-3 px-5 py-3 transition hover:bg-ink-50"
                  >
                    <div className="min-w-0">
                      <p className="truncate text-sm font-medium text-ink-900">{u.university}</p>
                      <p className="text-xs text-ink-500">
                        {u.totalSoldOut.toLocaleString()} sold out
                      </p>
                    </div>
                    <div className="flex shrink-0 items-center gap-2 text-sm font-semibold text-ink-900">
                      {u.totalAvailable.toLocaleString()}
